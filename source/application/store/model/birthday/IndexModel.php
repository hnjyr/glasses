...
namespace app\store\model\birthday;
 ...
 */
class IndexModel extends NewOrderModel
{
    /**
     * 订单列表
...
//        dump($query);die();
    ...
    private function setWhere($query)
    {
        ...
        if (isset($query['start_time']) && !empty($query['start_time'])) {
            $data = date('m-d',strtotime($query['start_time']));

            $this->where("DATE_FORMAT(order.birthday,'%m-%d') >=  '".$data."'");

        }
        if (isset($query['end_time']) && !empty($query['end_time'])) {
            $data = date('m-d',strtotime($query['end_time']));
            $this->where("DATE_FORMAT(order.birthday,'%m-%d') <  '".$data."'");
        }
