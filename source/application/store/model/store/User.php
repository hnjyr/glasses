...
class User extends StoreUserModel
{
    ...
    {
        // 验证用户名密码是否正确
//        dump($this->getLoginUser($data['user_name']));die();
...
//        dump($user['wxapp']);die();
        ...
        $shop_name = Db::name('user')->where('user_id',$user['user_id'])->find();
        $user['shop_name'] = $shop_name['shop_name'];
        // 保存登录状态
