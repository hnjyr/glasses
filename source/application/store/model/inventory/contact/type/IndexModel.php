...
namespace app\store\model\inventory\contact\type;
...
//        dump($query);die();
        ...
        return $this
            ->alias('contact_type')
            ->field('contact_type.*,user.shop_name,glasses_brand.brand_name')
    ...
    public function getType($user_id = null , $brand_id = null)
    {
        return $this
            ->alias('contact_type')
            ...
            ->where('contact_model.user_id',$user_id)
            ->where('contact_type.brand_id',$brand_id)
            ->where('contact_type.is_delete',0)
            ->order(['contact_type.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    }
