...
namespace app\store\model\inventory\contact;
...
class IndexModel extends BaseModel
{
    ...
    public function getList($dataType, $query = [])
    {
        ...
        return $this
            ->alias('contact_brand')
            ->field('contact_brand.*,user.shop_name')
            ->join('user', 'user.user_id = contact_brand.user_id')
            ->where($this->transferDataType($dataType))
            ->order(['contact_brand.create_time' => 'desc'])
            ->paginate(10, false, [
...
//        dump($query);
       ...
       return $this
            ...
            ->field('contact_brand.*,user.shop_name')
            ->join('user', 'user.user_id = contact_brand.user_id')
            ->where('contact_brand.user_id','in',$arr)
           ->where('contact_brand.is_delete',0)
            ->order(['contact_brand.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    ...
    public function add($data)
    {
        ...
        try {
            // 新增管理员记录
            $data['create_time'] = date("Y-m-d",strtotime(time()));
