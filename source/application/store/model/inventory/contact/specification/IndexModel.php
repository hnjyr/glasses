...
namespace app\store\model\inventory\contact\specification;
...
{
    protected $name = 'contact_specification';
     ...
     */
    public function getColor($user_id = null,$brand_id = null,$modal = null,$type = null)
    {

        return $this
            ->alias('contact_specification')
            ->field('contact_specification.*,user.shop_name,contact_brand.brand_name,contact_model.model')
            ->join('contact_brand','contact_brand.brand_id = contact_specification.brand_id')
            ->join('user', 'user.user_id = contact_specification.user_id')
            ->join('contact_model', 'contact_model.model_id = contact_specification.model_id')
            ->where('contact_specification.user_id',$user_id)
            ->where('contact_model.model',$modal)
            ->where('contact_specification.type',$type)
            ->where('contact_specification.brand_id',$brand_id)
            ->order(['contact_specification.create_time' => 'desc'])
            ->paginate(10, false, [
    ...
    public function getSpecification($user_id = null,$brand_id = null,$modal = null,$degree = null,$type = null,$color = null )
    {
    ...
    public function getLists($query = [],$arr=[],$type = null,$brand_id = null,$model_id= null,$color_id = null)
    {
        // 检索查询条件
//        dump($query);
        !empty($query) && $this->setWhere($query);
        // 获取数据列表
        if (is_null($model_id) && !is_null($brand_id) && is_null($type) && is_null($color_id)){
            return $this
                ->alias('contact_specification')
                ->field('contact_specification.*,user.shop_name,contact_brand.brand_name,contact_model.model')
                ->join('contact_brand','contact_brand.brand_id = contact_specification.brand_id')
                ->join('user', 'user.user_id = contact_specification.user_id')
                ->join('contact_model', 'contact_model.model_id = contact_specification.model_id')
                ->where('contact_specification.is_delete',0)
                ->where('contact_specification.user_id','in',$arr)
                ->where('contact_specification.brand_id',$brand_id)
                ->where('contact_specification.degree','<>','null')
                ->order(['contact_specification.create_time' => 'desc'])
                ->paginate(10, false, [
                    'query' => \request()->request()
                ]);
        }else if (is_null($model_id) && !is_null($brand_id) && !is_null($type) && is_null($color_id)){
            return $this
                ->alias('contact_specification')
                ->field('contact_specification.*,user.shop_name,contact_brand.brand_name,contact_model.model')
                ->join('contact_brand','contact_brand.brand_id = contact_specification.brand_id')
            ...
            return $this
                ...
                ->where('contact_specification.is_delete',0)
                ->where('contact_specification.user_id','in',$arr)
                ->where('contact_specification.model_id',$model_id)
                ->where('contact_specification.type',$type)
                ->where('contact_specification.degree','<>','null')
                ->where('contact_specification.brand_id',$brand_id)
                ->order(['contact_specification.create_time' => 'desc'])
            ...
            return $this
                ...
                ->where('contact_specification.model_id',$model_id)
                ->where('contact_specification.type',$type)
                ->where('contact_specification.color_id',$color_id)
                ->where('contact_specification.degree','<>','null')
                ->where('contact_specification.brand_id',$brand_id)
                ->order(['contact_specification.create_time' => 'desc'])
                ->paginate(10, false, [
            ...
            return $this
                ...
                ->where('contact_specification.is_delete',0)
                ->where('contact_specification.user_id','in',$arr)
                ->where('contact_specification.specification_id',$color)
                ->where('contact_specification.degree','<>','null')
                ->order(['contact_specification.create_time' => 'desc'])
                ->paginate(10, false, [
                    'query' => \request()->request()
