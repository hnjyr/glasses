...
namespace app\store\model\inventory\other\model;
...
class IndexModel extends BaseModel
{
     ...
     */
    public function getModel($user_id = null,$brand_id = null)
    {

            return $this
                ->alias('other_model')
                ->field('other_model.*,user.shop_name,other_brand.brand_name')
                ->join('other_brand','other_brand.brand_id = other_model.brand_id')
                ->join('user', 'user.user_id = other_model.user_id')
                ->where('other_model.user_id',$user_id)
                ->where('other_model.brand_id',$brand_id)
                ->order(['other_model.create_time' => 'desc'])
                ->paginate(10, false, [
...
//        dump($query);
        ...
        if (!is_null($brand_id)){
            return $this
                ...
                ->join('other_brand','other_brand.brand_id = other_model.brand_id')
                ->join('user', 'user.user_id = other_model.user_id')
                ->where('other_model.user_id','in',$arr)
                ->where('other_model.brand_id',$brand_id)
