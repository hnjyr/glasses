...
namespace app\store\model\inventory\other;
...
//        dump($query);
       ...
       return $this
            ->alias('other_brand')
            ->field('other_brand.*,user.shop_name')
            ->join('user', 'user.user_id = other_brand.user_id')
            ->where('other_brand.user_id','in',$arr)
           ->where('other_brand.is_delete',0)
            ->order(['other_brand.create_time' => 'desc'])
            ->paginate(10, false, [
