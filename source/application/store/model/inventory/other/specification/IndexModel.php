...
namespace app\store\model\inventory\other\specification;
...
{
    protected $name = 'other_specification';
    ...
    public function getSpecification($user_id = null,$model= null,$brand_id = null)
    {

            return $this
                ->alias('other_specification')
                ->field('other_specification.*,user.shop_name,other_brand.brand_name,other_model.model')
                ->join('other_brand','other_brand.brand_id = other_specification.brand_id')
                ->join('user', 'user.user_id = other_specification.user_id')
                ->join('other_model', 'other_model.model_id = other_specification.model_id')

                ->where('other_specification.user_id',$user_id)
                ->where('other_model.model',$model)
                ->where('other_specification.brand_id',$brand_id)
                ->order(['other_specification.create_time' => 'desc'])
                ->paginate(10, false, [
    ...
    //    dump($query);die;
        ...
        if (!is_null($model_id) && !is_null($brand_id)){
            return $this
                ...
                ->join('user', 'user.user_id = other_specification.user_id')
                ->join('other_model', 'other_model.model_id = other_specification.model_id')
                ->where('other_model.is_delete',0)
                ->where('other_specification.is_delete',0)
                ->where('other_specification.user_id','in',$arr)
                ->where('other_specification.model_id',$model_id)
                ->where('other_specification.brand_id',$brand_id)
                ->order(['other_specification.create_time' => 'desc'])
                ->paginate(10, false, [
                    'query' => \request()->request()
