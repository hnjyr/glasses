...
namespace app\store\model\inventory\index\refractive;
...
//        dump($query);die();
        ...
        return $this
            ->alias('refractive')
            ->field('refractive.*,user.shop_name,brand.brand_name')
            ->join('brand','brand.brand_id = refractive.brand_id')
            ->join('user', 'user.user_id = brand.user_id')
            ->where('is_delete',0)
    ...
    public function getLists($query = [],$arr=[],$type=null,$brand_id = null)
    {
        // 检索查询条件
//        dump($query);
        !empty($query) && $this->setWhere($query);
        ...
        } else if (!is_null($type) && !is_null($brand_id)){
        return $this
            ->alias('refractive')
            ->field('refractive.*,user.shop_name,brand.brand_name')
            ->join('brand','brand.brand_id = refractive.brand_id')
...
//        }
    ...
    public function getRefractive($type = null, $user_id = null , $brand_id = null){
        return  $this
            ->alias('refractive')
            ->field('refractive.*,user.shop_name,brand.brand_name')
            ->join('brand','brand.brand_id = refractive.brand_id')
            ->join('user', 'user.user_id = refractive.user_id')
            ->where('refractive.type',$type)
            ->where('refractive.user_id',$user_id)
            ->where('refractive.brand_id',$brand_id)
            ->order(['refractive.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
