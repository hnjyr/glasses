...
namespace app\store\model\inventory\index\specification;
...
//        dump($query);die();
        ...
        return $this
            ->alias('specification')
            ->field('specification.*,user.shop_name,brand.brand_name,refractive.refractive_num,model.model')
            ->join('brand','brand.brand_id = specification.brand_id')
            ->join('user', 'user.user_id = specification.user_id')
            ->join('refractive', 'refractive.refractive_id = specification.refractive_id')
            ->join('model', 'model.model_id = specification.model_id')
            ->where($this->transferDataType($dataType))
            ->order(['specification.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    }

    public function getrespecification($data = [])
    {
        return $this
            ->alias('specification')
            ->field('specification.*,user.shop_name,brand.brand_name,refractive.refractive_num,model.model')
            ...
            ->where('specification.user_id',$data['user_id'])
            ->where('specification.type',$data['type'])
            ->where('specification.brand_id',$data['brand'])
            ->where('model.model',$data['model'])
            ->where('refractive.refractive_num',$data['refractive'])
            ->where('specification.spherical_lens',$data['spherical_lens'])
            ->where('specification.cytdnder',$data['cytdnder'])
            ->order(['specification.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    ...
    public function getLists($query = [],$arr=[],$type= null,$brand_id = null,$model_id = null,$refractive_id = null)
    {
        // 检索查询条件
   
        !empty($query) && $this->setWhere($query);
        // 获取数据列表
        if (is_null($brand_id) && is_null($type) && is_null($model_id) && is_null($refractive_id)){
            return $this
                ->alias('specification')
                ->field('specification.*,user.shop_name,brand.brand_name,refractive.refractive_num,model.model')
                ->join('brand','brand.brand_id = specification.brand_id')
                ...
                ->join('refractive', 'refractive.refractive_id = specification.refractive_id')
                ->join('model', 'model.model_id = specification.model_id')
                ->where('specification.is_delete',0)
                ->where('specification.user_id','in',$arr)
        ...
        if (!is_null($brand_id) && is_null($type) && is_null($model_id) && is_null($refractive_id)){
            return $this
                ...
                ->where('specification.is_delete',0)
                ->where('specification.user_id','in',$arr)
                ->where('specification.brand_id',$brand_id)
        ...
        if (!is_null($brand_id) && !is_null($type) && !is_null($model_id) && is_null($refractive_id)){
            return $this
                ...
                ->where('specification.brand_id',$brand_id)
                ->where('specification.type',$type)
                ->where('specification.brand_id',$model_id)
                ->order(['specification.create_time' => 'desc'])
                ->paginate(10, false, [
                    'query' => \request()->request()
                ]);
        ...
        if (!is_null($brand_id) && !is_null($type) && !is_null($model_id) && !is_null($refractive_id)){
            return $this
                ...
                ->where('specification.brand_id',$brand_id)
                ->where('specification.type',$type)
                ->where('specification.model_id',$model_id)
                ->where('specification.refractive_id',$refractive_id)
                ->order(['specification.create_time' => 'desc'])
                ->paginate(10, false, [
                    'query' => \request()->request()
                ]);
