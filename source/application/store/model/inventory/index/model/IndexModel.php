...
namespace app\store\model\inventory\index\model;
...
//        dump($query);die();
        ...
        return $this
            ->alias('model')
            ->field('model.*,user.shop_name,brand.brand_name,refractive.refractive_num')
            ->join('brand','brand.brand_id = model.brand_id')
            ->join('user', 'user.user_id = model.user_id')
            ->join('refractive', 'refractive.refractive_id = model.refractive_id')
            ->where($this->transferDataType($dataType))
            ->where('is_delete',0)
            ->order(['model.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    }
    public function getModel($type = null, $user_id = null , $brand_id = null,$refractive_id = null)
    {
        ...
        return $this
            ...
            ->where('refractive.refractive_num',$refractive_id)
            ->order(['model.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    ...
    public function getLists($query = [],$arr=[],$type= null,$brand_id = null , $refractive_id = null)
    {
        // 检索查询条件
//        dump($query);
        !empty($query) && $this->setWhere($query);
        // 获取数据列表
        if (is_null($type) && !is_null($brand_id) &&  is_null($refractive_id)){
            return $this
                ->alias('model')
                ->field('model.*,user.shop_name,brand.brand_name,refractive.refractive_num')
                ->join('brand','brand.brand_id = model.brand_id')
                ...
                ->join('refractive', 'refractive.refractive_id = model.refractive_id')
                ->where('model.is_delete',0)
                ->where('model.user_id','in',$arr)
                ->where('model.brand_id',$brand_id)
                ->order(['model.create_time' => 'desc'])
                ->paginate(10, false, [
                    'query' => \request()->request()
                ]);
        }else if (!is_null($type) && !is_null($brand_id) &&  is_null($refractive_id)){
            return $this
                ->alias('model')
                ->field('model.*,user.shop_name,brand.brand_name,refractive.refractive_num')
                ->join('brand','brand.brand_id = model.brand_id')
            ...
            return $this
                ...
                ->where('model.type',$type)
                ->where('model.brand_id',$brand_id)
                ->where('model.refractive_id',$refractive_id)
                ->order(['model.create_time' => 'desc'])
                ->paginate(10, false, [
                    'query' => \request()->request()
                ]);
