...
namespace app\store\model\inventory\glasses\specification;
...
{
    protected $name = 'glasses_specification';
...
//        dump($query);die();
        ...
        // 获取数据列表
        return $this
            ->alias('glasses_specification')
            ->field('glasses_specification.*,user.shop_name,brand.brand_name,model.model')
            ->join('glasses_brand','glasses_brand.brand_id = glasses_specification.brand_id')
            ->join('glasses_user', 'glasses_user.user_id = glasses_specification.user_id')
            ->join('glasses_model', 'glasses_model.model_id = glasses_specification.model_id')
            ->where($this->transferDataType($dataType))
            ->order(['glasses_specification.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    }
    public function getColor($user_id = null ,$brand_id = null ,$modal = null)
    {

        return $this
            ->alias('glasses_specification')
            ->field('glasses_specification.*,user.shop_name,glasses_brand.brand_name,glasses_model.model')
            ->join('glasses_brand','glasses_brand.brand_id = glasses_specification.brand_id')
            ->join('user', 'user.user_id = glasses_specification.user_id')
            ->join('glasses_model', 'glasses_model.model_id = glasses_specification.model_id')
            ->where('glasses_specification.user_id',$user_id)
            ->where('glasses_specification.brand_id',$brand_id)
            ->where('glasses_model.model',$modal)
            ->order(['glasses_specification.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    }
    public function getSpecification($user_id = null ,$brand_id = null ,$modal = null,$glasses_color = null)
    {
        ...
        return $this
            ...
            ->where('glasses_specification.user_id',$user_id)
            ->where('glasses_specification.brand_id',$brand_id)
            ->where('glasses_specification.color',$glasses_color)
            ->where('glasses_model.model',$modal)
            ->order(['glasses_specification.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
    ...
    public function getLists($query = [],$arr=[],$model_id= null,$brand_id = null)
    {
        // 检索查询条件
//        dump($query);
        !empty($query) && $this->setWhere($query);
        // 获取数据列表
        if (!is_null($model_id) && !is_null($brand_id)){
            return $this
                ->alias('glasses_specification')
                ->field('glasses_specification.*,user.shop_name,glasses_brand.brand_name,glasses_model.model')
                ->join('glasses_brand','glasses_brand.brand_id = glasses_specification.brand_id')
                ->join('user', 'user.user_id = glasses_specification.user_id')
                ->join('glasses_model', 'glasses_model.model_id = glasses_specification.model_id')
                ->where('glasses_specification.is_delete',0)
                ->where('glasses_specification.user_id','in',$arr)
                ->where('glasses_specification.model_id',$model_id)
                ->where('glasses_specification.brand_id',$brand_id)
                ->order(['glasses_specification.create_time' => 'desc'])
                ->paginate(10, false, [
                    'query' => \request()->request()
