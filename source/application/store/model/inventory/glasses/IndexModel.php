...
namespace app\store\model\inventory\glasses;
...
class IndexModel extends BaseModel
{
    ...
    public function getList($dataType, $query = [])
    {
        ...
        return $this
            ->alias('glasses_brand')
            ->field('glasses_brand.*,user.shop_name')
            ->join('user', 'user.user_id = glasses_brand.user_id')
            ->where($this->transferDataType($dataType))
            ->order(['glasses_brand.create_time' => 'desc'])
            ->paginate(10, false, [
...
//        dump($query);
       ...
       return $this
            ...
            ->field('glasses_brand.*,user.shop_name')
            ->join('user', 'user.user_id = glasses_brand.user_id')
            ->where('glasses_brand.user_id','in',$arr)
           ->where('glasses_brand.is_delete',0)
            ->order(['glasses_brand.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
