...
namespace app\store\model\inventory\glasses\model;
...
//        dump($query);die();
        ...
        return $this
            ->alias('glasses_model')
            ->field('glasses_model.*,user.shop_name,glasses_brand.brand_name')
            ->join('glasses_brand','glasses_brand.brand_id = glasses_model.brand_id')
            ->join('user', 'user.user_id = model.user_id')
            ->where($this->transferDataType($dataType))
            ->order(['glasses_model.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    }

    public function getModel($user_id = null, $brand = null)
    {
        ...
        return $this
            ...
            ->field('glasses_model.*,user.shop_name,glasses_brand.brand_name')
            ->join('glasses_brand','glasses_brand.brand_id = glasses_model.brand_id')
            ->join('user', 'user.user_id = glasses_model.user_id')
            ->where('glasses_model.user_id',$user_id)
            ->where('glasses_model.brand_id',$brand)
            ->order(['glasses_model.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
...
//        dump($query);
        ...
        if (!is_null($brand_id)){
            return $this
                ...
                ->join('glasses_brand','glasses_brand.brand_id = glasses_model.brand_id')
                ->join('user', 'user.user_id = glasses_model.user_id')
                ->where('glasses_model.user_id','in',$arr)
                ->where('glasses_model.brand_id',$brand_id)
                ->where('glasses_model.is_delete',0)
                ->order(['glasses_model.create_time' => 'desc'])
                ->paginate(10, false, [
                    'query' => \request()->request()
                ]);
