...
//        dump($query);
    ...
    public function getCheckedLists($arr=[])
    {
        ...
        return $this
            ...
            ->field('contact_order.*,user.shop_name')
            ->join('user', 'user.user_id = contact_order.user_id')
            ->where('contact_order.order_num','in',$arr)
            ->where('contact_order.is_delete', '=', 0)
            ->order(['contact_order.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
    ...
    public function getOrderList($query,$page=0,$list)
    {
        ...
        return $this
            ...
            ->join('user', 'user.user_id = contact_order.user_id')
            ->where('contact_order.is_delete', '=', 0)
            ->where('contact_order.user_id', 'in', $list)
            ->page($page,15)
            ->order(['contact_order.create_time' => 'desc'])
...
//        }
    ...
    private function setWhere1($query)
    {
        if (isset($query['search']) && !empty($query['search']) ) {
            $this->where('contact_order.sales', 'like', '%' . trim($query['search']) . '%');
        }
    }
    private function setWhere2($query)
    {
        if (isset($query['search']) && !empty($query['search']) ) {
            $this->where('contact_order.user_name', 'like', '%' . trim($query['search']) . '%');
        }
    }
    /**
     * 转义数据类型条件
