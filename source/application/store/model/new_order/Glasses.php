 ...
 */
class Glasses extends BaseModel
{
    protected $name = 'glasses_order';
    /**
     * 订单列表
    ...
    public function getInfoList($dataType, $query = [])
    {
        // 检索查询条件
        !empty($query) && $this->setWhere($query);
        // 获取数据列表
...
//            dump($query['end_time']);die();
    ...
    public function getLists($query = [],$arr=[])
    {
        // 检索查询条件

        !empty($query) && $this->setWhere1($query);
        // 获取数据列表
       return $this
            ->alias('glasses_order')
            ->field('glasses_order.*,user.shop_name')
    ...
    public function getListss($query = [],$arr=[])
    {
        // 检索查询条件
...
//            dump($query['end_time']);die();
    ...
    public function getCheckedLists($arr=[])
    {
        ...
        return $this
            ...
            ->field('glasses_order.*,user.shop_name')
            ->join('user', 'user.user_id = glasses_order.user_id')
            ->where('glasses_order.order_no','in',$arr)
            ->where('glasses_order.is_delete', '=', 0)
            ->order(['glasses_order.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
    ...
    private function setWhere($query)
    {
        ...
        if (isset($query['search']) && !empty($query['search']) && !(isset($query['shopName']) && !empty($query['shopName']))) {
            $this->where('glasses_no|user_name|glasses_order.mobile', 'like', '%' . trim($query['search']) . '%');
        }
        if (isset($query['sales']) && !empty($query['sales']) ) {
            $this->where('glasses_order.sales', 'like', '%' . trim($query['sales']) . '%');
        }
        if (isset($query['search']) && !empty($query['search']) ) {
            $this->where('glasses_order.user_name', 'like', '%' . trim($query['search']) . '%');
        }
        if (isset($query['shopName']) && !empty($query['shopName']) && isset($query['search']) && !empty($query['search'])){
            $this->where('user.shop_name','like','%'.trim($query['shopName']).'%')->where('glasses_no|user_name|glasses_order.mobile', 'like', '%' . trim($query['search']) . '%');;
        }
        if (isset($query['start_time']) && !empty($query['start_time'])) {
            $this->where('glasses_order.create_time', '>=', strtotime($query['start_time']));
        }
        if (isset($query['end_time']) && !empty($query['end_time'])) {
            $this->where('glasses_order.create_time', '<', strtotime($query['end_time']) + 86400);
        }
        ...
        if (isset($query['user_id']) && !empty($query['user_id'])) {
            $this->where('glasses_order.user_id', 'in',$query['user_id']);
        }
        if (isset($query['s_time']) && !empty($query['e_time'])) {
            $this->where('glasses_order.create_time', 'between',[strtotime($query['s_time']),strtotime($query['e_time'])+86400]);
        }
        // 用户id
...
//        }
    ...
    private function setWhere1($query)
    {
        if (isset($query['search']) && !empty($query['search']) ) {
            $this->where('glasses_order.sales', 'like', '%' . trim($query['search']) . '%');
        }
    }
    private function setWhere2($query)
    {
        if (isset($query['search']) && !empty($query['search']) ) {
            $this->where('glasses_order.user_name',   trim($query['search']) );
        }
    }
    /**
     * 转义数据类型条件
...
//            'order_status' => ['<>', OrderStatusEnum::CANCELLED],
    ...
    {
        if (!is_null($startDate) && !is_null($endDate)) {
            $this->where('create_time', '>=', strtotime($endDate))
...
//            'order_status' => ['<>', OrderStatusEnum::CANCELLED],
   ...
   public function mFristAndLast($y = "", $m = "")
    ...
    {
        !empty($query) && $this->setWheres($query);
        return  $this->where('is_delete',0)->field('sales,pay_total,user_id')
