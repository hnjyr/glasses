...
class Other extends BaseModel
{
    ...
    public function getList($dataType, $query = [])
    {
        ...
        return $this
            ->alias('other_order')
            ->field('other_order.*,user.shop_name')
            ->join('user', 'user.user_id = other_order.user_id')
            ->where($this->transferDataType($dataType))
            ->where('other_order.is_delete', '=', 0)
            ->order(['other_order.create_time' => 'desc'])
            ->paginate(10, false, [
...
//        dump($query);
       ...
       return $this
            ...
            ->field('other_order.*,user.shop_name')
            ->join('user', 'user.user_id = other_order.user_id')
            ->where('other_order.user_id','in',$arr)
            ->where('other_order.is_delete', '=', 0)
            ->order(['other_order.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
...
//        dump($query);
        ...
        return $this
            ->alias('other_order')
            ->field('other_order.*,user.shop_name,sum(other_order.pay_total) as pay_other_total,sum(other_order.point) as other_total_point')
            ->join('user', 'user.user_id = other_order.user_id')
            ->where('other_order.user_id','in',$arr)
            ->where('other_order.is_delete', '=', 0)
            ->group('other_order.user_name')
            ->order(['other_order.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
            ]);
    ...
    public function getCheckedLists($arr=[])
    {
        ...
        return $this
            ...
            ->field('other_order.*,user.shop_name')
            ->join('user', 'user.user_id = other_order.user_id')
            ->where('other_order.order_num','in',$arr)
            ->where('other_order.is_delete', '=', 0)
            ->order(['other_order.create_time' => 'desc'])
            ->paginate(10, false, [
                'query' => \request()->request()
    ...
    private function setWhere($query)
    {
        ...
        if (isset($query['search']) && !empty($query['search']) && !(isset($query['shopName']) && !empty($query['shopName']))) {
            $this->where('other_no|user_name|other_order.mobile', 'like', '%' . trim($query['search']) . '%');
        }
        if (isset($query['shopName']) && !empty($query['shopName']) && isset($query['search']) && !empty($query['search'])){
            $this->where('user.shop_name','like','%'.trim($query['shopName']).'%')->where('other_no|user_name|other_order.mobile', 'like', '%' . trim($query['search']) . '%');;
        }
        if (isset($query['start_time']) && !empty($query['start_time'])) {
            $this->where('other_order.create_time', '>=', strtotime($query['start_time']));
        }
        if (isset($query['end_time']) && !empty($query['end_time'])) {
            $this->where('other_order.create_time', '<', strtotime($query['end_time']) + 86400);
        }
        ...
        if (isset($query['user_id']) && !empty($query['user_id'])) {
            $this->where('other_order.user_id', 'in',$query['user_id']);
        }
        if (isset($query['s_time']) && !empty($query['e_time'])) {
            $this->where('other_order.create_time', 'between',[strtotime($query['s_time']),strtotime($query['e_time'])+86400]);
        }
        // 用户id
