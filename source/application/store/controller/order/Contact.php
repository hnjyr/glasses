...
use app\store\model\order\contact\ContactModel as ContactModel;
...
class Contact extends Controller
{
    ...
    {
        return $this->getList('隐形眼镜订单列表', 'all');
    }

    public function del_order()
    {
        $contact = $this->request->param();
        $contact_id = $contact['contact'];
        $model = new ContactModel();
//        dump($contact_id);die();
        $res = Db::name('contact_order')->where('contact_id',$contact_id)->update(['is_delete'=>1]);
        if ($res){
            return $this->renderSuccess('删除成功',url('order.contact/all_list'));
        }
    }
    /**
     * 订单详情
    ...
    {
        // 订单详情
        $detail = ContactModel::detail($order_id);
    ...
    {
        $list = Db::name('contact_order')->where('contact_id',$order_id)->find();
