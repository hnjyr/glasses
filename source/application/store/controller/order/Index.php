...
class Index extends Controller
{
    ...
    private function getList($title, $dataType)
    {
        ...
        if ($admin_info['is_super'] == 1){
            ...
            $glassesList = $glassesModel->getList($dataType, $this->request->param());
            $contactList = $contactModel->getList($dataType, $this->request->param());
            $otherList = $otherModel->getList($dataType, $this->request->param());
            // 自提门店列表
            $this->assign('admin_info',$admin_info);
            return $this->fetch('index', compact('title', 'dataType', 'glassesList', 'contactList','otherList'));
        }else{
            $this_user = Db::name('user')->where(['pid'=>$admin_info['user_id']])->column('user_id');
            if(!empty($this_user)){
                $glassesModel = new GlassesModel;
                ...
                $otherList = $otherModel->getLists($this->request->param(),$this_user);
            }else{
                $glassesModel = new GlassesModel;
                $contactModel = new ContactModel;
                $otherModel = new OtherModel;
                ...
                $otherList = $otherModel->getLists($this->request->param(),$admin_info['user_id']);
            }
            // 自提门店列表
            $this->assign('admin_info',$admin_info);
            return $this->fetch('index', compact('title', 'dataType', 'glassesList', 'contactList','otherList'));
