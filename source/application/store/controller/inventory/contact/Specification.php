...
namespace app\store\controller\inventory\contact;

use app\store\model\inventory\contact\specification\IndexModel as IndexModel;
...
//        dump($this->request->get());
        ...
        return $this->getList('库存列表', 'all',$type,$model_id,$brand_id);
    }
    ...
    {
        $brand_id = $this->request->post('contact_brand');
        $modal = $this->request->post('contact_model');
        $type = $this->request->post('contact_type');
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();

        $model = new IndexModel;
        return $model->getColor($admin_info['user_id'],$brand_id,$modal,$type);
    }
    ...
    public function getspecification()
    {
        $brand_id = $this->request->post('contact_brand');
        $modal = $this->request->post('contact_model');
        $degree = $this->request->post('contact_degree');
        $type = $this->request->post('contact_type');
        $contact_color = $this->request->post('contact_color');
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();

//        dump($this->request->param());die;
        $model = new IndexModel;
        return $model->getSpecification($admin_info['user_id'],$brand_id,$modal,$degree,$type,$contact_color);
    }
     ...
     * @throws \think\exception\DbException
     */
    private function getList($title, $dataType,$type = null ,$model_id= null,$brand_id = null)
    {
        // 订单列表
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();
        ...
        if(!empty($this_user)){
            array_push($this_user,$admin_info['user_id']);
            $list = $model->getLists($this->request->param(),$this_user,$type,$model_id,$brand_id);
        }else{
            $list = $model->getLists($this->request->param(),$admin_info['user_id'],$type,$model_id,$brand_id);
        }
            // 自提门店列表
...
//        dump($this->postData());die();
        ...
        $data['standard_inventory'] = $data['standard_inventory'];
        $data['now_inventory'] = $data['standard_inventory'];

        if ($model->add($data)) {
            return $this->renderSuccess('添加成功', url('inventory.contact.specification/index&&type='.$data['type'].'&&brand_id='.$data['brand_id'].'&&model_id='.$data['model_id']));
        }
