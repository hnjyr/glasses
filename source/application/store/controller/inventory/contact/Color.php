...
use app\store\model\inventory\contact\color\IndexModel as IndexModel;
...
//        dump($this->request->get('type'));
        ...
        return $this->getList('型号列表', 'all',$type,$brand_id);
    }
    ...
    public function getcolor()
    {
        ...
        $model_id = $this->request->post('contact_model');
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();


        $model = new IndexModel;
        return $model->getColor($type,$admin_info['user_id'],$brand_id,$model_id);
    }
...
//        dump($this->postData());die();
        ...
        if ($model->add($data)) {
            return $this->renderSuccess('添加成功', url('inventory.contact.model/index&&type='.$data['type'].'&&brand_id='.$data['brand_id']));
        }
