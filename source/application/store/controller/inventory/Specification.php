...
use app\store\model\inventory\index\specification\IndexModel as IndexModel;
...
//        dump($this->request->get());
        ...
        return $this->getList('库存列表', 'all',$type,$brand_id);
    }
    ...
    public function getrespecification(){
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();
        $model = new IndexModel;
        $data = $this->postData();
        $data['user_id'] = $admin_info['user_id'];
        return $model->getrespecification($data);
    }
     ...
     * @throws \think\exception\DbException
     */
    private function getList($title, $dataType,$type= null,$brand_id = null)
    {
        // 订单列表
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();
        ...
        if(!empty($this_user)){
            array_push($this_user,$admin_info['user_id']);
            $list = $model->getLists($this->request->param(),$this_user,$type,$brand_id);
        }else{
            $list = $model->getLists($this->request->param(),$admin_info['user_id'],$type,$brand_id);
        }
            // 自提门店列表
...
//        dump($this->postData());die();
        ...
        $data['refractive_id'] = $data['refractive_id'];
        $data['model_id'] = $data['model_id'];
        $data['price'] = $data['price'];
        $data['spherical_lens'] = $data['spherical_lens'];
        $data['cytdnder'] = $data['cytdnder'];

        $data['standard_inventory'] = $data['standard_inventory'];
        $data['now_inventory'] = $data['standard_inventory'];

        if ($model->add($data)) {
            return $this->renderSuccess('添加成功', url('inventory.specification/index&&type='.$data['type'].'&&brand_id='.$data['brand_id'].'&&refractive_id='.$data['refractive_id'].'&&model_id='.$data['model_id']));
        }
