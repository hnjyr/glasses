...
use app\store\model\inventory\index\refractive\IndexModel as IndexModel;
 ...
 */
class Refractive extends Controller
{
    ...
    public function index()
    {
        ...
        if (is_null($type) || is_null($brand_id)){
            return $this->renderError($model->getError() ?:'非法访问');
        }
        return $this->getList('折射率列表', 'all',$type,$brand_id);
    }
    ...
    public function getrefractive()
    {
        $type = $this->request->post('type');
        $brand_id = $this->request->post('brand');
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();

        $model = new IndexModel;
        return $model->getRefractive($type,$admin_info['user_id'],$brand_id);
    }
    ...
    private function getList($title, $dataType,$type=null,$brand_id = null)
    {
        ...
        if(!empty($this_user)){
            array_push($this_user,$admin_info['user_id']);
            $list = $model->getLists($this->request->param(),$this_user,$type,$brand_id,$model_id);
        }else{
            $list = $model->getLists($this->request->param(),$admin_info['user_id'],$type,$brand_id,$model_id);
        }
            // 自提门店列表
...
//        $data['model_id'] = $data['model_id'];
        $data['refractive_num'] = $data['refractive_num'];
        if ($model->add($data)) {
            return $this->renderSuccess('添加成功', url('inventory.refractive/index&&type='.$data['type'].'&&brand_id='.$data['brand_id']));
        }
