...
use app\store\model\inventory\glasses\specification\IndexModel as IndexModel;
...
//        dump($this->request->get());
        ...
        return $this->getList('库存列表', 'all',$model_id,$brand_id);
    }
    ...
    public function getcolor()
    {
        ...
        $modal = $this->request->post('glasses_model');
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();

        $model = new IndexModel;
        return $model->getColor($admin_info['user_id'],$brand_id,$modal);
    }
    public function getspecification()
    {
        $brand_id = $this->request->post('glasses_brand');
        $modal = $this->request->post('glasses_model');
        $glasses_color = $this->request->post('glasses_color');
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();

        $model = new IndexModel;
        return $model->getSpecification($admin_info['user_id'],$brand_id,$modal,$glasses_color);
    }
     ...
     * @throws \think\exception\DbException
     */
    private function getList($title, $dataType,$model_id= null,$brand_id = null)
    {
        // 订单列表
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();
        ...
        if(!empty($this_user)){
            array_push($this_user,$admin_info['user_id']);
            $list = $model->getLists($this->request->param(),$this_user,$model_id,$brand_id);
        }else{
            $list = $model->getLists($this->request->param(),$admin_info['user_id'],$model_id,$brand_id);
        }
            // 自提门店列表
...
//        dump($this->postData());die();
        ...
        if ($model->add($data)) {
            return $this->renderSuccess('添加成功', url('inventory.glasses.specification/index&&brand_id='.$data['brand_id'].'&&model_id='.$data['model_id']));
        }
