...
use app\store\model\inventory\index\model\IndexModel as IndexModel;
...
//        dump($this->request->get('type'));
    ...
    public function getmodel()
    {
        ...
        $refractive_id = $this->request->post('refractive');

        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();

        $model = new IndexModel;
        return $model->getModel($type,$admin_info['user_id'],$brand_id,$refractive_id);
    }
...
//        dump($this->postData());die();
        ...
        if ($model->add($data)) {
            return $this->renderSuccess('添加成功', url('inventory.model/index&&type='.$data['type'].'&&brand_id='.$data['brand_id'].'&&refractive_id='.$data['refractive_id']));
        }
