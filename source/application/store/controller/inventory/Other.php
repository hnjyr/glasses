...
use app\store\model\inventory\other\IndexModel as IndexModel;
use app\store\model\inventory\other\model\IndexModel as ModelIndexModel;
use app\store\model\inventory\other\specification\IndexModel as SpecIndexModel;
...
class Other extends Controller
{
    ...
    public function getbrandlist(){
        ...
        if(!empty($this_user)){
            ...
            $brandlist = $model->getLists($this->request->param(),$this_user);
        }else{
            $brandlist = $model->getLists($this->request->param(),$admin_info['user_id']);
        }
        // 自提门店列表
    ...
    public  function getmodellist (){
            ...
            if(!empty($this_user)){
                array_push($this_user,$admin_info['user_id']);
                $modelList = $model->getLists($this->request->param(),$this_user,$brand_id);
            }else{
                $modelList = $model->getLists($this->request->param(),$admin_info['user_id'],$brand_id);
            }
            // 自提门店列表
            $this->assign('admin_info',$admin_info);
    ...
    public  function getspeclist (){
        ...
        if(!empty($this_user)){
            array_push($this_user,$admin_info['user_id']);
            $modelList = $model->getLists($this->request->param(),$this_user,$model_id,$brand_id);
        }else{
            $modelList = $model->getLists($this->request->param(),$admin_info['user_id'],$model_id,$brand_id);
        }
        // dump(db('specification')->getLastsql());die;
    ...
    public function del_brand(){
        ...
        $res = Db::name('other_brand')->where('brand_id',$brand_id)->update(['is_delete'=>1]);
        if ($res){
    ...
    public function del_model(){
        ...
        $res = Db::name('other_model')->where('model_id',$model_id)->update(['is_delete'=>1]);
        if ($res){
            return $this->renderSuccess('删除成功');
        }
        return $this->renderError('删除失败');
    ...
    public function del_spec(){
        ...
        $res = Db::name('other_specification')->where('specification_id',$specification_id)->update(['is_delete'=>1]);
        if ($res){
            return $this->renderSuccess('删除成功');
        }
        return $this->renderError('删除失败');
    ...
    public function update_brand(){
        ...
        if (!empty($this_user)){
            array_push($this_user,$admin_info['user_id']);
            $res = Db::name('other_brand')
                ->where('brand_id',$brand_id)
                ->where('user_id','in',$this_user)
    ...
    public function add()
    {
        ...
        if ($model->add($data)) {
            return $this->renderSuccess('添加成功', url('inventory.other/index'));
        }
