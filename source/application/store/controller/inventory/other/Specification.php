...
namespace app\store\controller\inventory\other;

use app\store\model\inventory\other\specification\IndexModel as IndexModel;
...
//        dump($this->request->get());
    ...
    {
        $brand_id = $this->request->post('other_brand');
        $modal = $this->request->post('other_model');
        $admin_info = Db::name('store_user')->where(['store_user_id'=>Session::get('yoshop_store')['user']['store_user_id']])->find();

        $model = new IndexModel;
        return $model->getSpecification($admin_info['user_id'],$modal,$brand_id);
    }
...
//        dump($this->postData());die();
        ...
        if ($model->add($data)) {
            return $this->renderSuccess('添加成功', url('inventory.other.specification/index&&brand_id='.$data['brand_id'].'&&model_id='.$data['model_id']));
        }
