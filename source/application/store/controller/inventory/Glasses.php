...
use app\store\model\inventory\glasses\IndexModel as IndexModel;
use app\store\controller\Controller;
use app\store\model\inventory\glasses\model\IndexModel as ModelIndexModel;
use app\store\model\inventory\glasses\specification\IndexModel as SpecIndexModel;
...
class Glasses extends Controller
{
    ...
    public function del_brand(){
        ...
        $res = Db::name('glasses_brand')->where('brand_id',$brand_id)->update(['is_delete'=>1]);
        if ($res){
    ...
    public function del_model(){
        ...
        $res = Db::name('glasses_model')->where('model_id',$model_id)->update(['is_delete'=>1]);
        if ($res){
            return $this->renderSuccess('删除成功');
        }
        return $this->renderError('删除失败');
    ...
    public function del_spec(){
        ...
        $res = Db::name('glasses_specification')->where('specification_id',$specification_id)->update(['is_delete'=>1]);
        if ($res){
            return $this->renderSuccess('删除成功');
        }
        return $this->renderError('删除失败');
    ...
    public function add()
    {
        ...
        if ($model->add($data)) {
            return $this->renderSuccess('添加成功', url('inventory.glasses/index'));
        }
