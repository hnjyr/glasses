...
namespace app\store\controller\sales;
...
class History extends Controller
{
    ...
    {
        return $this->getList('历史记录', 'all',$this->request->param('user_id'));
    }
     ...
     * @throws \think\exception\DbException
     */
    private function getList($title, $dataType,$user_id = null)
    {
        // 订单列表
        ...
        if ($admin_info['is_super'] == 1){
            ...
            $glasseslist = $glassesmodel->getLists($dataType, $this->request->param());
            $contactlist = $contactmodel->getLists($dataType, $this->request->param());
            $list = $model->getList($dataType, $this->request->param());
            // 自提门店列表
            $this->assign('admin_info',$admin_info);
            $model = new StoreModel();

            $data = $model->getHomeData($user_id);
            return $this->fetch('index', compact('title', 'dataType', 'list','data', 'glasseslist','contactlist'));
        }else{
            $model = new IndexModel();
            $glassesmodel = new GlassesModel();
            $contactmodel = new ContactModel();
            if(!empty($this_user)){
                ...
                $glasseslist = $glassesmodel->getLists($this->request->param(),$this_user);
                $contactlist = $contactmodel->getLists($this->request->param(),$this_user);
//                dump($this_user);die();
                $model = new StoreModel();
                $data = $model->getHomeData($user_id);
            }else{
                ...
                $glasseslist = $glassesmodel->getLists($this->request->param(),$admin_info['user_id']);
                $contactlist = $contactmodel->getLists($this->request->param(),$admin_info['user_id']);
                $model = new StoreModel();
//                dump($admin_info['user_id']);die();
                $data = $model->getHomeData($user_id);
            }
            // 自提门店列表
            $this->assign('admin_info',$admin_info);
