...
class Goods extends BaseModel
{
    ...
    public function getList($param)
    {
        ...
        !empty($params['goods_type']) && $params['goods_type'] > 0 && $filter['goods_type'] = $params['goods_type'];
        !empty($params['search']) && $filter['goods_name'] = ['like', '%' . trim($params['search'] . '%')];
        !empty($params['is_try']) && $filter['is_try'] = 1;
        !empty($params['is_new']) && $filter['is_new'] = 1;
        !empty($params['share']) && $filter['share'] = ['not null', ''];
        // 排序规则
        $sort = [];
    ...
    public function getList2($param)
    {
        // 商品列表获取条件
        $params = array_merge([
            'status' => 10,         // 商品状态
        ...
        !empty($params['search']) && $filter['goods_name'] = ['like', '%' . trim($params['search']) . '%'];
        !empty($params['is_try']) && $filter['is_try'] = 1;
        !empty($params['page'])  && $page = $params['page'];
        // 排序规则
        $sort = [];
        if ($params['sortType'] === 'all') {
            $sort = ['goods_sort', 'goods_id' => 'desc'];
        ...
        $list = $this
            ...
            ->where('is_delete', '=', 0)
            ->where($filter)
            ->order($sort)->limit(15)->page($page)->select();
//            ->paginate($params['listRows'], false, [
//                'query' => \request()->request()
//            ]);
        // 整理列表数据并返回
