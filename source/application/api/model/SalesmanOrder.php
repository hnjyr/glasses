...
use app\common\model\SalesmanOrder as SalesmanOrderModel;
 ...
 */
class SalesmanOrder extends SalesmanOrderModel
{
    /**
     * 隐藏字段
     * @var array
...
//    }
    ...
    public function onOrderPayment($user, $order, $payType)
    {
        ...
        if ($payType != 10) {
            $data =  $this->pay($user, $order['order_no'], $payType);
            if($payType ==  20){
                if(!$data['returnmsg']){
                    $sign = $data['prepayid'];
                    $pop['sign'] = $sign;
                    $pop['create_time'] = $time;
                    $pop['order_no'] = $order['order_no'];
                    Db::name('paysign')->insert($pop);
                    $data = ['prepayid'=>$order['order_no']];
                }
            }
            return ['type'=>$payType,'data'=>$data];
        }
    ...
    public function pay($user,$order_no,$type)
    {
        ...
        $url = 'http://'.$_SERVER['HTTP_HOST'].'/index.php?s=/api/page/notify_url';
        $op = [
