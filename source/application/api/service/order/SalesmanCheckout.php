...
class SalesmanCheckout
{
    ...
    {
        $this->model = new SalesmanOrderModel;
        $this->wxapp_id = 10001;
    }
...
//        dump($this->validateGoodsList());die;
    ...
    private function setOrderExpress()
    {
        ...
        // 当前用户收货城市id
        $cityId = $this->user['address_id'] ? $this->user['address_id'] : null;
        if($cityId){
            $cityId =  Db::name('user_address')->where(['address_id'=>$cityId])->value('city_id');
        }
        // 初始化配送服务类
        if($cityId){
            $ExpressService = new ExpressService($cityId, $this->goodsList, OrderTypeEnum::MASTER);
        // 验证商品是否在配送范围
        if (!$isIntraRegion = $ExpressService->isIntraRegion()) {
    ...
    private function createOrderEvent($order)
    {
        ...
        // 更新商品库存 (针对下单减库存的商品)
        if($order['spec_type'] == 10){
          $this->updateGoodsStockNum($order);
        }

        if($order['spec_type'] == 10){
            $this->updateGoodsStockNum2($order);
        }
...
//        }
        // 获取订单详情
//        $detail = SalesmanOrderModel::getUserOrderDetail($this->model['order_id'], $this->user['user_id']);
        // 记录分销商订单
...
//        return Db::name('salesman_order_goods')->insertAll($goodsList);
    }
     ...
     * @return mixed
     */
    private function updateGoodsStockNum2($order)
    {
        return StockFactory::getFactory($this->model['order_source'])->updateStockSales($order['goods_list']);
    }
...
//        return Db::name('order_address')->insert([
//            'user_id' => $this->user['salesman_id'],
//            'wxapp_id' => $this->wxapp_id,
//            'name' => $address['name'],
//            'phone' => $address['phone'],
