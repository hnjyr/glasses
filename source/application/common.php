...
function getStringToSign($parameter)
    {
        ksort($parameter);
        $parameter['key'] = getSign(); // 可以获取签名key
        $parameter['key'] = config('Sgin'); //给的默认key的值
        $array = [];
        ...
        $str1 =  stripcslashes($string); //拼接参数
        unset($parameter['key']);
        $signstr = strtoupper(md5($str1)); //md5 -- 转大写
...
function getPayData($parameter){
    $url = config('payurl');
    $sign = getStringToSign($parameter);
    $parameter['sign'] = $sign;
    // if($parameter['opt'] == 'upPreOrder'){
    //     return $parameter;
    // }
    // dump($parameter);die;
    $res = curlPost($url,$parameter);
    $res = json_decode($res,true);
    return $res;
}
...
function get_setting()
{
    ...
    $store = \think\Db::name('setting')->where('key','store')->find();
    $store = explode(',',$store['values']);
    $store = explode(':',$store[0]);
    return json_decode($store[1]);
}
